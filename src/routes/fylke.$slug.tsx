...
export const Route = createFileRoute("/fylke/$slug")({
  loader: async ({ params }) => {
    const { lastFylkeside } = await import("@/components/region/last");
    const side = await lastFylkeside(params.slug);
  ...
  head: ({ loaderData, params }) => {
    ...
    const beskrivelse =
      ...
      (r.totalt
        ? ` ${tall(r.verifisert)} av ${tall(r.totalt)} roller er hentet direkte fra registrene${fylke.grader.sist_hentet ? ` ${datoKort(fylke.grader.sist_hentet)}` : ""}.`
        : "");
...
function Fylkeside() {
  ...
  const varsel = varselForRoller(`i ${fylke.navn}`, fylke.grader.roller, fylke.grader.sist_hentet);
  const politiskeLedere = side.politisk.filter((o) => o.ledere.length);

  return (
    <>
      <Topplinje
        kommune={{ navn: fylke.navn, kommunenr: fylke.fylkesnr, nummertekst: `fylke ${fylke.fylkesnr}` }}
        varsel={varsel}
      />
      <main id="innhold">
        <section aria-labelledby="fylke-tittel" className="ramme pt-[clamp(28px,5vw,64px)] pb-[clamp(24px,4vw,48px)]">
          <div className="grid gap-x-6 gap-y-10 lg:grid-cols-12">
            <div className="flex min-w-0 flex-col gap-7 lg:col-span-5 lg:pt-3">
              ...
              <p className="ingress max-w-[38ch] text-[clamp(1.0625rem,1rem+0.35vw,1.25rem)] text-dempet">
                ...
                {tall(fylke.antall_kommuner)} kommunene i {fylke.navn}. Hver påstand har kilde og dato.
              </p>
              ...
              <div className="grid gap-6 md:grid-cols-2">
                <div className="flex min-w-0 flex-col gap-3">
                  ...
                  {politiskeLedere.length ? (
                    ...
                    <Organblokk organ={side.fylkeskommune} ledere={[]} tom="Fylkestinget er ikke kartlagt" />
                  )}
                </div>
                <div className="flex min-w-0 flex-col gap-3">
                  ...
                  {side.administrativ.length ? (
                    side.administrativ.map((o) => (
                      <Organblokk key={o.key} organ={o} tom="Administrativ leder ikke kartlagt" />
                    ))
                  ) : (
                    <Organblokk
                ...
                {side.andre.length > 0 && (
                  <p className="text-[0.875rem] leading-[1.5] text-dempet md:col-span-2">
                    Flere organer i fylket:{" "}
                    {side.andre.map((o, i) => (
                      <span key={o.key}>
                        {i > 0 && ", "}
                        <Link
